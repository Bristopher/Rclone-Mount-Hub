...
```bash
vpk pack --packId "Rclone Mount Hub" --packTitle "Rclone Mount Hub" --packVersion 0.1.2 --packDir "src-tauri/target/release" --mainExe "Rclone Mount Hub.exe"
```
...
2. Build + package:
   ```bash
   cd src-tauri && pnpm tauri build
   vpk pack --packId "Rclone Mount Hub" --packTitle "Rclone Mount Hub" --packVersion x.y.z --packDir "src-tauri/target/release" --mainExe "Rclone Mount Hub.exe"
   ```
