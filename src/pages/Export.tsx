...
export function Export() {
  ...
  const generatePowerShellScript = (conn: Connection): string => {
    const host = conn.network_mode === "tailscale"
      ? conn.tailscale_ip
      : conn.local_ip;
    const remoteType = conn.remote_type || "webdav";

    // Build protocol-specific flags
    let protocolFlags = "";
    switch (remoteType) {
      case "webdav":
        protocolFlags = `  --webdav-url "http://${host}:${conn.port}" \`\n  --webdav-user "${conn.username}"`;
        break;
      case "sftp":
        protocolFlags = `  --sftp-host "${host}" \`\n  --sftp-port "${conn.port}"`;
        if (conn.vendor === "sftpgo") {
          protocolFlags += ` \`\n  --sftp-concurrency 16 \`\n  --sftp-set-modtime false \`\n  --sftp-idle-timeout 60s`;
        }
        break;
      case "ftp":
        protocolFlags = `  --ftp-host "${host}" \`\n  --ftp-port "${conn.port}"`;
        break;
      case "smb":
        protocolFlags = `  --smb-host "${host}" \`\n  --smb-port "${conn.port}"`;
        break;
      default:
        protocolFlags = `  # ${remoteType} uses config-level settings`;
    }
